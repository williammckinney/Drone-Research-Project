...
float PID, yLeft, yRight, error, previous_error;
float pid_p=0;
float pid_i=0;
float pid_d=0;
/////////////////PID CONSTANTS/////////////////
double kp=0.2;//3.55
double ki=0.00378;//0.005
double kd=0.105;//2.05
///////////////////////////////////////////////
...
void setup() {
  ...
  right_servo.write(48);
  left_servo.attach(7);
  left_servo.write(147);
...
//  If file opened, write to it:
  if (myFile){
    ...
    myFile.println(String(Total_angle[1]) + ", "+  String(time/1000));
    Serial.println("Done");
    myFile.close();
  }else{
...
double xleft= (yLeft+220)/1.5;
double xright= -(yRight-73.6)/1.5225;

/*Finnaly using the servo function we create the PWM pulses with the calculated
width for each pulse*/
left_servo.write(xleft);
right_servo.write(xright);
